...
@section('content')
    <div class="content-wrapper">
        @include('patials.content_header', ['name' => 'Role', 'key' => 'Edit'])
        <div class="content">
                                            ...
                                            <div class="row">
                                                @foreach($perItem->permisstionchilr as $perchilItem)
                                                    <div class="card-body text-primary col-md-3" >
                                                        <h5 class="card-title">
                                                            <label for="">
                                                                <input type="checkbox" class="checkbox_chil"
                                                                       {{ $permisstioncheck->contains('id',$perchilItem->id ) ? 'checked' : ''}}
                                                                       name="permisstion_id[]"
                                                                       value="{{ $perchilItem->id }}">
                                                            </label>
                                                            {{ $perchilItem->name }}
                                                        </h5>
                                                    </div>
